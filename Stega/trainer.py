...
import trainer_encoder_decoder, trainer_detector
...
def train_stega_model():

    detector_model = load_model_detector()

    model_trainer = trainer_encoder_decoder.TrainerEncoderDecoder(LEARNING_RATE,
                                                                  BATCH_SIZE,
                                                                  EPOCHS,
                                                                  BETA,
                                                                  NORMALIZE,
...
def train_detector_model():
    ...
    model_trainer = trainer_detector.TrainerDetector(LEARNING_RATE,
                                                     BATCH_SIZE,
                                                     EPOCHS,
                                                     BETA,
                                                     NORMALIZE,
...
def test_model_detector(test_dir = Path("data/stega_dataset/test"), test_batch_size = 1):
    ...
    test_dataloader = dataset_loader.DatasetLoader.get_test_dataloader(test_dir, test_batch_size, 1, NORMALIZE)
...
def arg_paser_trainer_type():
    parser = argparse.ArgumentParser(description='Welcome to Deep Learning Image Steganography Trainer.')
    parser.add_argument('-t','--trainer', help='Trainer type: 0 for Steganography (Default), 1 for detector.', required=False)
